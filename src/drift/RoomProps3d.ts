...
import type { RoomArchetypeId } from '../game/world/RoomArchetypes';

export type RoomPropMeshes = Record<RoomArchetypeId, MeshData>;

export function buildRoomPropMeshes(): RoomPropMeshes {
  return {
...
function buildCrypt(): MeshBuilder {
...
function buildStorage(): MeshBuilder {
  const mesh = new MeshBuilder();
...
function buildArmory(): MeshBuilder {
  const mesh = new MeshBuilder();
...
function buildPrison(): MeshBuilder {
  const mesh = new MeshBuilder();
...
function buildRitual(): MeshBuilder {
  const mesh = new MeshBuilder();
...
function buildLibrary(): MeshBuilder {
  const mesh = new MeshBuilder();
...
function buildGuardRoom(): MeshBuilder {
  const mesh = new MeshBuilder();
...
function buildReliquary(): MeshBuilder {
  const mesh = new MeshBuilder();
...
function buildExtractionRig(): MeshBuilder {
  const mesh = new MeshBuilder();
