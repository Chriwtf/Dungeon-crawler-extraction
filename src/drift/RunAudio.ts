...
type RunSound = 'step' | 'turn' | 'door' | 'locked' | 'strike' | 'heavy' | 'guard' | 'dodge' | 'medkit' | 'container' | 'loot' | 'damage' | 'relic' | 'apex' | 'extract';
...
export class RunAudio {
  ...
  private ambient: ReturnType<AudioGraph['createLoop']> = null;
  private buffers: Partial<Record<RunSound, AudioBuffer>> = {};
  ...
  unlock(): void {
    ...
    if (this.initializing === null) this.initializing = this.initialize();
  }

  play(sound: RunSound): void {
    this.graph?.play(this.buffers[sound], sound === 'damage' || sound === 'apex' ? 0.55 : 0.34);
  }

  playTurn(action: 'move' | 'turn' | 'blocked' | 'torch' | 'door' | 'attack' | 'heavyAttack' | 'guard' | 'dodge' | 'item'): void {
    const sound: RunSound = action === 'move' ? 'step'
      : action === 'turn' ? 'turn'
  ...
  setTension(facilityMode: number, hunting: boolean, relicSecured: boolean): void {
