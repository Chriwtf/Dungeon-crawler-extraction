...
export async function startVerticalSlice() {
    ...
    const enemyRigNodes = new Map();
    const enemyAnimators = new Map();
    const syncEnemyNodes = (snapshots) => {
        for (const enemy of snapshots) {
            ...
            if (node === undefined) {
                ...
                if (rigged !== null)
                    enemyAnimators.set(enemy.id, createRiggedAnimator(rigged));
            }
    ...
    let apexPosition = dungeon.playerStart;
    ...
    if (minimapContext === null)
        throw new Error('Minimap canvas is unavailable.');
    ...
    startLoop({
        ...
        render(alpha) {
            ...
            for (let index = 0; index < dungeon.doors.length; index += 1) {
                if (!doorSystem.isBlocking(dungeon.doors[index].point) || !exploration.isVisible(dungeon.doors[index].point))
                    continue;
...
function isWalkable(dungeon, doors, enemies, position) {
...
function facilityReadout(mode) {
    return mode === 2 ? 'THE LIGHTS DIE' : mode === 1 ? 'RED LIGHTS FLICKER' : 'THE FACILITY HUMS';
}
function apexReadout(mode, relicSecured) {
    if (mode === 'hunting')
        return 'YOU ARE BEING HUNTED';
    if (mode === 'searching')
        return relicSecured ? 'SOMETHING FOLLOWS THE SIGNAL' : 'SOMETHING IS LOOKING';
    return 'THE DARK IS STILL';
}
function enemyAnimationState(state) {
    if (state === 'attack')
        return 'attack';
    return state === 'idle' ? 'idle' : 'move';
}
function apexAnimationState(mode) {
    return mode === 'dormant' ? 'idle' : mode === 'searching' ? 'move' : 'attack';
}
function meter(value, maximum) {
    const slots = 10;
...
function drawMinimap(context, dungeon, exploration, player, hasRelic, scale) {
...
function drawMinimapMarker(context, point, scale, color) {
