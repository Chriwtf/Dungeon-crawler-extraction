...
import type { LootKind } from '../game/core/RunLoot';

export type LootPropMeshes = Record<LootKind, MeshData>;

export function buildLootPropMeshes(): LootPropMeshes {
...
class TexturedPropBuilder {
  ...
  addCylinder(x: number, y: number, z: number, radius: number, height: number, segments: number): void {
    const y0 = y - height / 2;
    const y1 = y + height / 2;
    for (let index = 0; index < segments; index += 1) {
      ...
      this.addQuad(
        [p0, p1, p2, p3],
        [Math.sin((angle0 + angle1) / 2), 0, Math.cos((angle0 + angle1) / 2)],
        1 / segments,
        1,
        index / segments,
      );
  ...
  private addQuad(points: readonly number[][], normal: readonly number[], u = 1, v = 1, uOffset = 0): void {
    const at = this.positions.length / 3;
    for (const point of points) this.addVertex(point, normal);
    this.uvs.push(uOffset, 0, uOffset + u, 0, uOffset + u, v, uOffset, v);
    this.indices.push(at, at + 1, at + 2, at, at + 2, at + 3);
  }

  private addTriangle(points: readonly number[][]): void {
  ...
  private addVertex(point: readonly number[], normal: readonly number[]): void {
