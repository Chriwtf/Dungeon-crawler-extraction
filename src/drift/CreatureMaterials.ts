...
type CreatureMaterial = SurfaceMaterial<MaterialHandle>;
type MaterialProfile = { readonly roughness: number; readonly metallic: number; readonly normalStrength: number };
...
export async function loadCreatureMaterials(renderer: RendererApi): Promise<Record<CreatureKind, CreatureMaterial>> {
  ...
  return Object.fromEntries(entries) as Record<CreatureKind, CreatureMaterial>;
}

async function loadBitmap(url: string): Promise<ImageBitmap> {
  const response = await fetch(url);
  if (!response.ok) throw new Error(`Could not load creature material: ${url}`);
...
function materialCanvas(bitmap: ImageBitmap): HTMLCanvasElement {
  ...
  if (context === null) throw new Error('2D canvas is unavailable for creature material processing.');
...
function createOrmMap(bitmap: ImageBitmap, profile: MaterialProfile): HTMLCanvasElement {
  const canvas = materialCanvas(bitmap);
  const context = canvas.getContext('2d', { willReadFrequently: true });
