...
export class MenuScene extends Phaser.Scene {
  private rainTexts: Phaser.GameObjects.Text[] = [];
  ...
  create(): void {
    ...
    this.add
      .text(
        GAME_WIDTH / 2,
        228,
        'Terminale roguelike a turni. Recupera il reperto, estrai vivo, decidi se tornare o scendere piu a fondo.',
        {
  ...
  private createRain(): void {
    ...
    for (let index = 0; index < columns; index += 1) {
      const text = this.add.text(
        36 + index * 66,
        Phaser.Math.Between(-320, GAME_HEIGHT - 80),
        this.buildRainColumn(glyphs),
        {
  ...
  private buildRainColumn(glyphs: string): string {
