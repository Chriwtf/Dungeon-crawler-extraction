...
import { getRandomNeighborSteps, getStepTowardTarget, isAdjacent, isBossDepth, isFinalDepth, MAX_DUNGEON_DEPTH, rollDamage, spawnMonsters, } from '../combat/monsters';
import { TurnEngine } from '../core/TurnEngine';
import { applyItemEffect } from '../items/itemEffects';
import { BASE_INVENTORY_SIZE, cloneItem, getInventorySizeBonus, rollMonsterDrop, spawnGroundItems, } from '../items/inventory';
...
export class RunScene extends Phaser.Scene {
    constructor() {
        ...
        Object.defineProperty(this, "groundItems", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        Object.defineProperty(this, "inventory", {
            ...
            configurable: true,
            writable: true,
            value: Array(BASE_INVENTORY_SIZE).fill(null)
        });
        Object.defineProperty(this, "backpackSlot", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "isBossFloor", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        });
        Object.defineProperty(this, "isFinalFloor", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        Object.defineProperty(this, "bossMonsterId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "logTitleText", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "inventoryTitleText", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "statusTitleText", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "inventoryText", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "logScrollOffset", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "statusDetailScrollOffset", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "lastStatusDetailContent", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: ''
        ...
        Object.defineProperty(this, "playerStats", {
            enumerable: true,
            configurable: true,
            writable: true,
        ...
        Object.defineProperty(this, "completionState", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 'none'
        });
        Object.defineProperty(this, "currentDepth", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 1
        ...
        Object.defineProperty(this, "mapOriginX", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: MAP_AREA_X
        });
        Object.defineProperty(this, "mapOriginY", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: MAP_AREA_Y
    ...
    init(data) {
        ...
        const normalizedItems = this.normalizeInventory(data.startingInventory);
        this.inventory = normalizedItems.inventory;
        this.backpackSlot = normalizedItems.backpack;
    ...
    create() {
        ...
        this.logTitleText = this.add.text(SIDEBAR_X + 16, 61, 'SYS LOG // WHEEL', createTextStyle('15px', matrixPalette.text));
        ...
        this.statusTitleText = this.add.text(SIDEBAR_X + 16, DETAILS_PANEL_Y + 5, 'COMMANDS // DETAIL // WHEEL', createTextStyle('15px', matrixPalette.text));
        ...
        this.constrainTextToPanel(this.inventoryText, PANEL_BODY_X, INVENTORY_BODY_Y, PANEL_BODY_WIDTH, INVENTORY_BODY_HEIGHT);
        ...
        this.constrainTextToPanel(this.statusDetailText, DETAILS_BODY_X, DETAILS_FOOTER_Y, DETAILS_BODY_WIDTH, DETAILS_FOOTER_HEIGHT);
    ...
    normalizeInventory(source) {
        const sourceItems = (source ?? []).filter((item) => item !== null);
        const backpack = sourceItems.find((item) => item.category === 'backpack') ?? null;
        const inventoryItems = sourceItems.filter((item) => item.category !== 'backpack');
        const derivedSize = BASE_INVENTORY_SIZE + getInventorySizeBonus(backpack ? [backpack] : []);
        const targetSize = Math.max(BASE_INVENTORY_SIZE, derivedSize, source?.length ?? 0);
        const inventory = Array(targetSize).fill(null);
        if (!source) {
            return { inventory, backpack };
        }
        for (let index = 0; index < Math.min(inventoryItems.length, targetSize); index += 1) {
            inventory[index] = cloneItem(inventoryItems[index]);
        }
        return {
            inventory,
            backpack: backpack ? cloneItem(backpack) : null,
        };
    }
    getInventorySize() {
    ...
    getSelectableSlotCount() {
        return this.getInventorySize() + 1;
    }
    isBackpackSlotSelected() {
        return this.selectedSlot === this.getInventorySize();
    }
    getSelectedItem() {
        return this.isBackpackSlotSelected() ? this.backpackSlot : this.inventory[this.selectedSlot];
    }
    syncInventoryCapacity() {
        ...
        this.selectedSlot = Phaser.Math.Clamp(this.selectedSlot, 0, Math.max(0, this.getSelectableSlotCount() - 1));
    }
    canEquipBackpack(candidate) {
        const nextInventorySize = BASE_INVENTORY_SIZE + getInventorySizeBonus(candidate ? [candidate] : []);
        return this.getOccupiedSlots() <= nextInventorySize;
    }
    setupRun({ depth, carryInventory, carryHp, }) {
        ...
        const normalizedItems = this.normalizeInventory(carryInventory);
        this.inventory = normalizedItems.inventory;
        this.backpackSlot = normalizedItems.backpack;
        this.selectedSlot = 0;
        ...
        this.playerStats = derivePlayerStats(this.inventory, getRunBonusesForLevel(this.currentLevel), this.backpackSlot);
    ...
    bindInput() {
        ...
        this.input.on('wheel', (pointer, _currentlyOver, _deltaX, deltaY) => {
    ...
    isPointerInsidePanel(pointer, x, y, width, height) {
    ...
    syncStatusDetailScroll() {
        this.statusDetailScrollOffset = Phaser.Math.Clamp(this.statusDetailScrollOffset, 0, this.getMaxScroll(this.statusDetailText, DETAILS_FOOTER_HEIGHT));
        this.statusDetailText.setY(DETAILS_FOOTER_Y - this.statusDetailScrollOffset);
    }
    getMaxScroll(text, viewportHeight) {
    ...
    handleRunCompleteInput(code) {
        if (this.completionState === 'extracted') {
            ...
            if (code === 'KeyB') {
                depositInventoryToStash(this.inventory, this.backpackSlot);
                this.scene.start('base');
                return;
            }
        }
    ...
    pickUpItem() {
        ...
        if (groundItem.item.category === 'backpack') {
            if (!this.canEquipBackpack(groundItem.item)) {
                this.addLogLines(['Non puoi indossare questo zaino: perderesti slot ancora occupati.']);
                this.refreshHud();
                return;
            }
            const previousBackpack = this.backpackSlot;
            this.backpackSlot = groundItem.item;
            this.recalculatePlayerStats();
            this.selectedSlot = this.getInventorySize();
            if (previousBackpack) {
                groundItem.item = previousBackpack;
                this.resolvePlayerTurn([`Sostituisci lo zaino con ${this.backpackSlot.name}.`]);
                return;
            }
            this.groundItems = this.groundItems.filter((entry) => entry.item.id !== groundItem.item.id);
            this.resolvePlayerTurn([`Indossi ${this.backpackSlot.name} nello slot zaino.`]);
            return;
        }
    ...
    useSelectedItem() {
        ...
        if (!item) {
            this.addLogLines([this.isBackpackSlotSelected() ? 'Lo slot zaino e vuoto.' : `Lo slot ${this.selectedSlot + 1} e vuoto.`]);
            this.refreshHud();
            return;
        }
        if (item.category !== 'consumable') {
            this.addLogLines([
                item.category === 'backpack'
                    ? `${item.name} e passivo: resta attivo finche occupa lo slot zaino.`
                    : `${item.name} e passivo: il bonus resta finche occupa lo slot.`,
            ]);
            this.refreshHud();
            return;
        }
        ...
        if ((item.damageAmount ?? 0) > 0) {
            ...
            for (const monster of removedMonsters) {
                ...
                bonusLogLines.push(...this.handleMonsterDefeat(monster, this.grantExperience(monster.expReward)));
            }
        }
    ...
    dropSelectedItem() {
        ...
        if (item.category === 'backpack' && !this.canEquipBackpack(null)) {
    ...
    selectNextSlot() {
        this.selectedSlot = (this.selectedSlot + 1) % this.getSelectableSlotCount();
        this.addLogLines([this.isBackpackSlotSelected() ? 'Slot attivo: zaino.' : `Slot attivo: ${this.selectedSlot + 1}.`]);
        this.refreshHud();
    }
    ...
    selectBackpackSlot() {
        this.selectedSlot = this.getInventorySize();
        this.addLogLines(['Slot attivo: zaino.']);
        this.refreshHud();
    }
    isWalkable(x, y) {
    ...
    updatePlayerVisual() {
        this.playerRect.setPosition(this.mapOriginX + this.player.x * this.tileSize + this.tileSize / 2, this.mapOriginY + this.player.y * this.tileSize + this.tileSize / 2);
    }
    refreshHud() {
        this.hudText.setText([
            ...
            `Zaino: ${this.backpackSlot ? this.backpackSlot.name : '(nessuno)'}`,
    ...
    buildInventoryText() {
        const lines = [
            ...
            '',
            'SLOT ZAINO',
            `${this.isBackpackSlotSelected() ? '[>]' : '[ ]'} [0] ${this.backpackSlot ? `${this.backpackSlot.name}${this.getPassiveTag(this.backpackSlot)}` : '(vuoto)'}`,
            '',
            'INVENTARIO ATTIVO',
        ];
        ...
        lines.push('');
        lines.push('A TERRA');
        lines.push(groundItem ? `${groundItem.item.glyph} ${groundItem.item.name}` : '(nulla)');
        if (groundItem) {
            lines.push(groundItem.item.description);
        }
        return lines.join('\n');
    }
    buildStatusText() {
        ...
        return [
            ...
            'F usa consumabile',
            'Q lascia oggetto',
            `TAB / 0-${this.getInventorySize()} cambia slot`,
    ...
    buildStatusDetailText() {
        ...
        return [
            ...
            `SLOT ${this.isBackpackSlotSelected() ? 'ZAI  slot zaino' : selectedItem ? getItemAsciiLabel(selectedItem) : '...  (vuoto)'}`,
            selectedItem ? selectedItem.description : 'Seleziona uno slot pieno per usarlo.',
            `A TERRA ${groundItem ? `${groundItem.item.glyph}  ${groundItem.item.name}` : '(nulla)'}`,
            groundItem ? groundItem.item.description : 'Nessun oggetto ai tuoi piedi.',
    ...
    handleMonsterDefeat(monster, logLines) {
