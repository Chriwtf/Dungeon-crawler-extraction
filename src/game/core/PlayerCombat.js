/** Deterministic player-side combat state; enemies remain owned by the ECS director. */
export class PlayerCombat {
    constructor(seed) {
        Object.defineProperty(this, "maxHp", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 36
        });
        Object.defineProperty(this, "hpValue", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: this.maxHp
        });
        Object.defineProperty(this, "medkitsValue", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 2
        });
        Object.defineProperty(this, "stance", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 'none'
        });
        Object.defineProperty(this, "randomState", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        this.randomState = (seed ^ 0x51f15e) >>> 0 || 0x6d2b79f5;
    }
    get hp() { return this.hpValue; }
    get medkits() { return this.medkitsValue; }
    attack(heavy) {
        const hit = !heavy || this.roll() < 0.72;
        return { hit, damage: heavy ? 21 + Math.floor(this.roll() * 5) : 10 + Math.floor(this.roll() * 4) };
    }
    guard() { this.stance = 'guard'; }
    dodge() { this.stance = 'dodge'; }
    useMedkit() {
        if (this.medkitsValue <= 0 || this.hpValue >= this.maxHp)
            return 0;
        this.medkitsValue -= 1;
        const recovered = Math.min(15, this.maxHp - this.hpValue);
        this.hpValue += recovered;
        return recovered;
    }
    resolveIncoming(damage) {
        const stance = this.stance;
        this.stance = 'none';
        if (damage <= 0)
            return { damage: 0, avoided: false, guarded: false, defeated: false };
        if (stance === 'dodge' && this.roll() < 0.65)
            return { damage: 0, avoided: true, guarded: false, defeated: false };
        const applied = stance === 'guard' ? Math.max(1, Math.ceil(damage * 0.4)) : damage;
        this.hpValue = Math.max(0, this.hpValue - applied);
        return { damage: applied, avoided: false, guarded: stance === 'guard', defeated: this.hpValue === 0 };
    }
    roll() {
        let state = this.randomState;
        state ^= state << 13;
        state ^= state >>> 17;
        state ^= state << 5;
        this.randomState = state >>> 0;
        return this.randomState / 4294967296;
    }
}
