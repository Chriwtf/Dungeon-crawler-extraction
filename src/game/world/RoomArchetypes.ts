...
export type RoomArchetype = {
  readonly id: RoomArchetypeId;
  readonly minSize: number;
  readonly maxSize: number;
  readonly weight: number;
  readonly lootProfile: string;
  readonly enemyProfile: string;
  readonly propProfile: string;
  readonly lootChance: number;
  readonly enemyChance: number;
  readonly requirement?: string;
  readonly specialFunction?: string;
};

export type RoomArchetypeAssignment = {
  readonly index: number;
  readonly width: number;
  readonly height: number;
  readonly forcedId?: RoomArchetypeId;
};

export const ROOM_ARCHETYPES: Readonly<Record<RoomArchetypeId, RoomArchetype>> = {
  crypt: {
...
const STANDARD_ARCHETYPES: readonly RoomArchetypeId[] = ['crypt', 'storage', 'armory', 'prison', 'ritual', 'library', 'guardRoom'];

export function assignRoomArchetypes(assignments: readonly RoomArchetypeAssignment[], random: () => number): RoomArchetypeId[] {
  const assigned: RoomArchetypeId[] = [];
...
function supportsSize(archetype: RoomArchetype, area: number): boolean {
  return area >= archetype.minSize * archetype.minSize && area <= archetype.maxSize * archetype.maxSize;
}

function weightedPick(candidates: readonly RoomArchetypeId[], random: () => number): RoomArchetypeId {
